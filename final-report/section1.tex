...
% Possibly expand on deep learing approaches here
...
%- Discuss template comparison as a method to identify the fiducial markers. Template comparison is has been used to compare alphabetical and numerical character images \cite{Lin:Character_Cube_Stacking_Robot}. However, the major drawback of this is that each unknown image has to be compared with potentially all of the images in the reference database which is computationally expensive. 
...
%Furthermore, real world cameras have lens distortion effects that need to be adjusted for. The OpenCV library was used to perform this camera calibration process to produce the intrinsic parameters of the camera.
