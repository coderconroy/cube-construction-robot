...
In order to identify if there was any colour information within \FigRef{fig:original-cube-image}, a version of the image zoomed in on the cube was cropped to provide an even distribution of background area as well as cube area within the image. A histogram showing the RGB colour component distributions within the \FigRef{fig:original-cube-image} was generated as shown in \FigRef{fig:original-cube-close-hist}. The histogram shows the RGB components are all approximately aligned which means there is no significant colour information in the image, only grey intensity information. The histogram shows the pixel intensities are clustered around two intensity values. \FigRef{fig:original-cube-image} was converted to greyscale and had binary thresholding applied to generate the cube mask shown in \FigRef{fig:original-cube-mask}. Applying this mask to \FigRef{fig:original-cube-image} yields \FigRef{fig:original-cube-segment} showing a successfully segmented cube.
...
\begin{figure}[H]
   ...
   \end{subfigure}
    \captionsetup{singlelinecheck = false, justification=justified}
    \caption{Segmentation of cube in \FigRef{fig:original-cube-image} using grey intensity thresholding.}
    \label{fig:grey-cube-segment}
\end{figure}

\subsubsection{Colour Segmentation with Overhead Lighting}

Although the cube was successfully segmented as shown in \FigRef{fig:grey-cube-segment} the following points must be made:
...
\FigRef{fig:blue-cube-close-image} was created to provide a better representation of the colour component differences between the cube and the light blue background. \FigRef{fig:blue-cube-close-hist} shows the histogram showing the RGB component pixel intensity distribution in \FigRef{fig:blue-cube-close-image}. In order to produce more tightly coupled colour groups for segmentation purposes, a Gaussian blur function was applied to \FigRef{fig:blue-cube-close-image} from which the histogram shown in \FigRef{fig:blue-cube-close-hist-blur} was produced. This histogram shows that the red channel exhibits the clearest difference between the top face of the cube and the sides and background. A red channel binary threshold was therefore applied to \FigRef{fig:blue-cube-image} to produce the top cube face mask shown in \FigRef{fig:blue-cube-top-mask}. Canny edge detection was further applied to yield \FigRef{fig:blue-cube-top-canny} followed by the Hough Transform for straight lines to yield \FigRef{fig:blue-cube-top-hough}. The intended approach from this point would be to merge similar lines into a single line and identify the points of intersection as vertices which can then be used to identify the location and pose of the cube.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
         ...
         \caption{Zoomed in perspective of cube in \FigRef{fig:blue-cube-image}.}
         \label{fig:blue-cube-close-image}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\textwidth]{figures/202105/blue-cube-close-hist.jpg}
        \caption{Histogram showing the distribution of RGB component pixel intensities in \FigRef{fig:blue-cube-close-image} with the pixel intensity increasing linearly from left to right.}
        \label{fig:blue-cube-close-hist}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\textwidth]{figures/202105/blue-cube-close-hist-blur.jpg}
        \caption{Histogram showing the distribution of RGB component pixel intensities in \FigRef{fig:blue-cube-close-image} with Guassian blurring applied with the pixel intensity increasing linearly from left to right.}
        \label{fig:blue-cube-close-hist-blur}
    \end{subfigure}
    \captionsetup{singlelinecheck = false, justification=justified}
    \caption{Extraction of colour information from \FigRef{fig:blue-cube-image} to be used in segmentation of the cube.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \caption{Segmentation mask for top cube face in \FigRef{fig:blue-cube-image}.}
        \label{fig:blue-cube-top-mask}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\textwidth]{figures/202105/blue-cube-top-canny.jpg}
        \caption{Output of Canny edge detector applied to \FigRef{fig:blue-cube-top-mask}.}
        \label{fig:blue-cube-top-canny}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \includegraphics[width=\textwidth]{figures/202105/blue-cube-top-hough.jpg}
        \caption{Output of Hough Transform for straight line detection applied to \FigRef{fig:blue-cube-top-mask}.}
        \label{fig:blue-cube-top-hough}
    \end{subfigure}
    \captionsetup{singlelinecheck = false, justification=justified}
    \caption{Segmentation of top cube face in \FigRef{fig:blue-cube-image} and extraction of line information.}
